...
if ! [ -d "$HOME/.dotnet-core-tools" ]; then
	...
	dotnet tool install EasyDotnet --tool-path ~/.dotnet-core-tools

else
	echo ".dotnet-core-tools folder already exists, skipping"
fi

if ! [ -d "$HOME/.dotfiles/.config/LazyVim" ]; then
	...
	echo "LazyVim is already installed, skipping"
fi
